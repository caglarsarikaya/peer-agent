...
session management, and conversation history storage using async Motor driver.
"""
...
class MongoDB:
    """
    Async MongoDB interface for the Peer Agent system.
    
    Handles connections, CRUD operations, and session management using Motor.
    """
    ...
    async def _create_indexes(self):
        ...
        try:
            ...
            await self.interactions_collection.create_index("session_id")
            
            # Index on timestamp for chronological queries
            await self.interactions_collection.create_index([("timestamp", DESCENDING)])
            ...
            await self.interactions_collection.create_index("agent_type")
            
            # Session collection indexes
            await self.sessions_collection.create_index("session_id", unique=True)
            await self.sessions_collection.create_index([("last_interaction", DESCENDING)])
    ...
    async def save_interaction(self, interaction_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if result.inserted_id:
                ...
                await self._update_session(interaction_data.get("session_id"))
    ...
    async def _update_session(self, session_id: str):
        ...
        if not session_id or not await self.is_connected():
            return
        ...
        try:
            ...
            result = []
            async for doc in self.interactions_collection.aggregate(pipeline):
                result.append(doc)
    ...
    async def cleanup_old_sessions(self, days: int = 30) -> int:
        ...
        try:
            ...
            interactions_result = await self.interactions_collection.delete_many({
